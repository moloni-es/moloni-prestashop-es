...
class SettingsFormType extends TranslatorAwareType
{
    ...
    {
        $exemptions = $this->options->getExemptionReasons();
        ...
        if (empty($exemptions)) {
            ...
            $data['choices'] = $exemptions;
            $data['placeholder'] = $this->trans('Please select an option', 'Modules.Molonies.Settings');
        }

        $this->builder->add('exemptionReasonProduct', $type, $data);
    ...
    private function exemptionReasonShipping(): SettingsFormType
    {
        ...
        $this->builder->add('exemptionReasonShipping', $type, $data);
