...
class General extends FrameworkBundleAdminController
{
    ...
    public function checkLogin()
    {
        ...
        if (empty($array) || $array['company_id'] == '0' || $array['date_login'] == '') {
            Error::addError($this->trans('No user registered!!', 'Modules.Molonies.Errors'));
    ...
    public function checkTokens()
    {
        ...
        if (empty($dataArray) ||
            ...
            Log::writeLog($this->trans('Database is empty!!', 'Modules.Molonies.Errors'));
        ...
        if ($dataArray['refresh_expire'] < (time())) {
            Log::writeLog($this->trans('Refresh token has expired', 'Modules.Molonies.Errors'));
        ...
        if ($dataArray['access_expire'] < (time() + 300)) {
            ...
            if (!isset($newDataArray['errors'])) {
                ...
                Moloni::fillCache();
                Log::writeLog($this->trans('Tokens Refreshed', 'Modules.Molonies.Errors'));

                return true;
            } else {
                Log::writeLog($this->trans('Problem refreshing tokens!!', 'Modules.Molonies.Errors'));
                Error::addError($this->trans('Problem refreshing tokens!!', 'Modules.Molonies.Errors'));
    ...
    public static function getAllCategoriesFromMoloni($categoryParentId)
    {
        ...
        $categories = (Categories::queryProductCategories($variables));
